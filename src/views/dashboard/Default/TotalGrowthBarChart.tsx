...
export default function TotalGrowthBarChart({ isLoading }) {
  ...
  useEffect(() => {
    setChartOptions({
      ...
      xaxis: { ...barChartOptions.xaxis, style: { colors: textPrimary } },
      grid: { borderColor: divider },
      tooltip: { theme: 'light' },
      legend: { ...(barChartOptions.legend ?? {}), labels: { ...(barChartOptions.legend?.labels ?? {}), colors: grey500 }, show: true, position: 'bottom' as const }
    } as any);
      ...
      {isLoading ? (
        ...
        <MainCard>
          <Stack sx={{ gap: gridSpacing }}>
            ...
            >
              <Chart options={chartOptions as any} series={series} type="bar" height={480} />
            </Box>
          </Stack>
        </MainCard>
      )}
